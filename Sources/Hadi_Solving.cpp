...
void print(Circuit *C) {
  if (C->IsEmpty())
    ...
    Node *Head = C->GetFirstNode();
    while (Head != NULL) {
      cout << "The " << Head->GetId() << " Node Has Volt: " << Head->GetVolt()
           << endl;
      Element *temp = Head->GetFirstElement();
      while (temp != NULL) {
        cout << "The Element Type Is: " << temp->GetType()
             << "\tAnd Id: " << temp->GetId()
             << "\tAnd val: " << temp->GetValue() << "\n";
        temp = temp->GetNext();
      }
      Head = Head->GetNext();
    }
      ...
      while (e) {
        ...
        if (!tempE) {
          ...
          while (tempN) {
            ...
            while (eCom2) {
              if (eCom->GetType() == eCom2->GetType() &&
                  eCom->GetId() == eCom2->GetId()) {
                arr[i][j] += -1 / series;
                break;
              }
              eCom2 = eCom2->GetNext();
